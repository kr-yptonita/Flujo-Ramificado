...
En su trayecto, los fotones cambiarán de vecino siguiendo siempre el camino más corto posible evaluado por la red. Como ya se demostró matemáticamente en las secciones previas, recordaremos que en este sistema la luz no puede reflejarse (no hay retrorreflexión), por lo que el avance siempre mantiene un frente progresivo. Adicionalmente, se presenta una consideración física fundamental para el modelo: en caso de que el paso más corto se encuentre en una región con el mismo espesor (múltiples opciones óptimas o un medio homogéneo), el haz de fotones no se desviará al azar, sino que seguirá rigurosamente la misma dirección que tenía en el paso anterior, conservando así su momento.
...
La implementación algorítmica en serie mediante estructuras dinámicas convencionales (diccionarios de Python) arroja un tiempo de cálculo aproximado de $70$ segundos por fotón. Trazar un haz compuesto por $4$ fotones (simulando 4 píxeles de grosor) para la totalidad de los $847$ fotogramas extraídos exigiría más de $65$ horas de cómputo ininterrumpido. Para viabilizar la experimentación, es esencial implementar una optimización estructural de memoria: el reemplazo de las estructuras de rastreo por matrices indexadas tridimensionales (`NumPy`) permite un acceso directo a memoria contigua, mitigando los cuellos de botella del algoritmo. Asimismo, es importante señalar que, al resolver estados independientes para múltiples rayos en cientos de imágenes paralelas, este modelo algorítmico es inherentemente masivo y altamente acelerable bajo cómputo de GPU.
